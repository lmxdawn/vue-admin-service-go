...
func (pc *PermissionCreate) SetPermissionRuleID(i int) *PermissionCreate {
	pc.mutation.SetPermissionRuleID(i)
	return pc
}
...
func (pc *PermissionCreate) check() error {
	...
	if _, ok := pc.mutation.PermissionRuleID(); !ok {
		return &ValidationError{Name: "permission_rule_id", err: errors.New(`ent: missing required field "Permission.permission_rule_id"`)}
	}
...
func (pc *PermissionCreate) createSpec() (*Permission, *sqlgraph.CreateSpec) {
	...
	if value, ok := pc.mutation.PermissionRuleID(); ok {
		_spec.Fields = append(_spec.Fields, &sqlgraph.FieldSpec{
			Type:   field.TypeInt,
			Value:  value,
			Column: permission.FieldPermissionRuleID,
		})
		_node.PermissionRuleID = value
	}
